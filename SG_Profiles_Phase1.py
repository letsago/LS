...
planning_group_list = ['KOHLS']
style_list = ['511', '505', '505BT', '501', '502', '510', '512', '513', '514', '514BT', '527', '541', '541BT', '550', '550BT', '559', '559BT', '569']
avgWithinSS = {}
    ...
    # Assign PC9 volume clusters to unique dataframes
        ...
        for cluster in cluster_categories:
            ...
            if cluster == max_cluster:
                ...
                if cluster_sum[df_name] > 95000:
                    percent_retention[df_name] = 0.99
                if cluster_sum[df_name] <= 95000 and cluster_sum[df_name] >= 45000:
                    percent_retention[df_name] = 0.988
                if cluster_sum[df_name] < 45000:
                    percent_retention[df_name] = 0.98
                for i in range(number_rows[df_name]):
                    if grouped_data[df_name].Size_Shipped_Qty[i] < (1 - percent_retention[df_name]) * cluster_sum[df_name]:
                        counter = counter + 1
                    else:
                        break
        ...
        constraint_data = {}
        temp_data_2 = {}
        final_cluster_categories = []
        final_cluster_categories.append(max(cluster_categories))
        ...
        # combines into one higher volume cluster size grid if size count differnce between consecutive size grids is 3 or fewer 
        for i in list(reversed(range(1, len(cluster_categories)))):
            df_name_1 = 'PC9_volume_cluster_' + str(i)
            df_name_2 = 'PC9_volume_cluster_' + str(i - 1)
            if len(constraint_data[df_name_1]) - len(constraint_data[df_name_2]) > 3:
                final_cluster_categories.append(i - 1)
        
        # export one output file
        
        for cluster in final_cluster_categories:
            df_name = 'PC9_volume_cluster_' + str(cluster)
